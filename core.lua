...
-----------------------------------------------------------------------------------------------------------------------------------------
Native = {}
Tunnel.bindInterface("garages", Native)
...
-- PAYMENTSTORE
-----------------------------------------------------------------------------------------------------------------------------------------
function Native.PaymentStore(Number)
	local source = source
	local Passport = vRP.Passport(source)
...
-- SERVERVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Native.ServerVehicle(Model, x, y, z, Heading, Plate, Nitrox, Doors, Body, Fuel)
...
-- VEHICLES
-----------------------------------------------------------------------------------------------------------------------------------------
function Native.Vehicles(Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not exports["hud"]:Reposed(Passport, source) and not exports["hud"]:Wanted(Passport, source) then
...
-- SPAWN
-----------------------------------------------------------------------------------------------------------------------------------------
function Native.Spawn(Name, Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
					...
					if Coords then
						...
						if Garages[Number]["payment"] then
							if vRP.UserPremium(Passport) then
								...
								local Exist, Network = Native.ServerVehicle(Name, Coords[1], Coords[2], Coords[3], Coords[4], Plate, vehicle[1]["Nitro"], vehicle[1]["Doors"], vehicle[1]["Body"])
...
RegisterCommand("car", function(source, Message)
	...
	if Passport then
		if vRP.HasGroup(Passport, CarPermission) and Message[1] then
			...
			local Exist, Network, Vehicle = Native.ServerVehicle(VehicleName, Coords["x"], Coords["y"], Coords["z"], Heading, Plate, 2000, nil, 1000)
			if not Exist then
				return
			end
...
-- DELETE
-----------------------------------------------------------------------------------------------------------------------------------------
function Native.Delete(Network, Health, Engine, Body, Fuel, Doors, Windows, Tyres, Plate, Brake)
	if Spawn[Plate] then
